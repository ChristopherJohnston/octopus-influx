...
class OctopusEnergy:
    ...
    def get_tariff_data(self, resource_type, charge_type, period_from=None, period_to=None, page_size=100, page=None):
        ...
        :rtype: dict
        """
        period_from_str = f"&period_from={_iso_format(period_from)}" if period_from is not None else ""
        period_to_str = f"&period_to={_iso_format(period_to)}" if period_to is not None else ""
    ...
    def get_consumption_for_period(self, resource_type, period_from, period_to, reverse_order=False, page_size=100, page=None, group_by=Aggregate.HALF_HOURLY):
        ...
        return self.get_data(f"{self.consumption_url(resource_type)}?period_from={_iso_format(period_from)}&period_to={_iso_format(period_to)}&order_by={order}&page_size={page_size}{page_str}{group_by_str}")
...
def main():
    ...
    logger.info(octopus_client.get_grid_supply_points("GU22 7LA"))
